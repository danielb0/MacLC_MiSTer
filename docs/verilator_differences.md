...
| **Floppy mounts** (2026-09-14, Phase 1 of the write plan) | `ioctl_download` blob into SDRAM, index 1/2, with its own copy of the DC42 strip and the download-edge media-change logic (`dio_menu`, `old_down`) | **hps_io block-device slots `VD_FLOPPY_INT`=6 / `VD_FLOPPY_EXT`=7** (`S6`/`S7` OSD entries) streamed by `rtl/floppy_loader.v`; DC42 strip and media change both moved into/onto the mount (`img_mounted` -> `loading` -> `done`) | **The sim cannot exercise `floppy_loader`, the mount-driven media change, the DC42 strip as shipped, or the download-port arbiter at all.** Covered instead by `verilator/tb_floppy_loader.v` (19 checks, incl. the DC42 offset) and `verilator/tb_disk_swap.v` (22 checks, the CSTIN/SWITCHED transitions). ★ The sim harness already models 3 block-device slots in C++ (`sd_*[0..2]`), so mirroring this is FEASIBLE, just not free: it needs 2 more slots plus image feeding. Do it if floppy behaviour ever needs a full-core sim; until then this row IS the divergence notice. |
| **Medium sidedness** `mediaSides` (2026-09-19, plan Phase 6B) | hardwired `2'b11` at the `dataController_top` instance — sim.v has no `floppy_loader`, so there is no mount-time volume sniff to do | `{1'b1, flp_int_media_ds}`, the loader's sniff of the MDB in sector 2 | 1 is "unknown", the term that never lowers the sidedness ceiling, so sim behaves exactly as before. **The One-Sided-erase-survives-a-remount path is not reachable in sim at all**; it is covered by `verilator/tb_floppy_loader.v` section 6 and, on hardware, by the One-Sided erase gate. |
