/*
 PORTED VERBATIM from ../MacPlus_MiSTer/rtl/floppy_track_decoder.v on
 2026-09-15 (source md5 6fcd1267e0d1765847c7d1009763848a). Only this header
 block was added; not one line of logic was touched.

 That is deliberate, and it is the whole value of the port. This core's
 rtl/floppy_track_encoder.v is MacPlus's file MINUS the format relay -- every
 differing line between the two is inside the relay block (re-diffed
 2026-09-15) -- so the decoder below is the algebraic inverse of the exact
 stream THIS core emits, already validated on a sister core. Any local
 "improvement" trades that inheritance for a fresh unknown. Change it only
 with the Phase 2 round-trip bench (verilator/tb_floppy_track_decoder.v)
 green, and port the change back to MacPlus so the two do not drift.

 Note on `addr` and format neutrality (plan section 5, Phase 2): the GCR
 geometry (soff/spt, the variable-speed zones) lives HERE rather than in the
 committer, so the committer stays format-neutral -- it consumes
 (sector_valid, sector, addr, buf) and never learns which encoding produced
 it. Stage 2's MFM decoder presents the same tuple, computing its own flat
 18-sector geometry. Moving addr into the committer would make the committer
 format-aware, which is exactly what the carve-out exists to avoid.
 */
/*
